...
def DenseLayer(x, nb_filter, bn_size=4, alpha=0.0, drop_rate=0.2):
    ...
    x = SeparableConv2D(bn_size*nb_filter, (1, 1), strides=(1,1), padding='same')(x)
 
    # Composite function
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=alpha)(x)
    x = SeparableConv2D(nb_filter, (3,3), strides=(1,1), padding='same')(x)
...
def TransitionLayer(x, compression=0.5, alpha=0.0, is_max=0):
    ...
    x = BatchNormalization()(x)
    x = LeakyReLU(alpha=alpha)(x)
    x = SeparableConv2D(nb_filter, (1, 1), strides=(1,1), padding='same')(x)
...
x = Conv2D(growth_rate*2, (3, 3), strides=1, padding='same')(inputs)#1
x = BatchNormalization()(x)
...
improved_defined = Model(inputs, outputs, name="improved_defined")
improved_defined.compile(loss=tf.keras.losses.SparseCategoricalCrossentropy(),
...
# 顯示類神經網路架構
improved_defined.summary()
#畫出類神經網路架構 存到improved_defined.png裡
tf.keras.utils.plot_model(improved_defined,to_file="improved_defined.png",dpi=96) 

#訓練及顯示訓練過程
history = improved_defined.fit_generator(
                              train_generator , 
                              epochs=35,
...
#存訓練模型
improved_defined.save("improved_defined_model.h5")

#使用已儲存之模型 #改良版為improved_defined_model.h5，未改良版為self_defined_model
#improved_defined = load_model("improved_defined_model.h5") 
...
y_predictions = improved_defined.predict(X) #回傳各圖之三類機率，需以下式過濾出機率最大的類別
